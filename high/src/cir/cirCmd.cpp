...
//                [-Output (string logFile)][-Mode (int mode)]
//----------------------------------------------------------------------
CmdExecStatus
CirSimCmd::exec(const string& option)
{
   if (cirMgr -> proof() == true ){
      cerr<<"All gates are proved."<<endl;
      return CMD_EXEC_ERROR;
   }
   if (!cirMgr) {
      cerr << "Error: circuit is not yet constructed!!" << endl;
   ...
   ofstream logFile;
   bool doRandom = false, doFile = false, doLog = false, _mode = false;
   int mode = 0;
   int simlim = -1;
   for (size_t i = 0, n = options.size(); i < n; ++i) {
      ...
      else if (myStrNCmp("-Mode", options[i], 2) == 0) {
         if ( _mode )
            return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[i]);
         if (++i == n)
            return CmdExec::errorOption(CMD_OPT_MISSING, options[i-1]);
         if ( !( myStr2Int(options[i], mode) && mode > 0 && mode <= 5 ) ) 
            return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[i]);
         switch ( mode ){
            case 1:
               simlim = 20;
   ...
   cirMgr -> setlimit( simlim );
   if ( doRandom ){
      if ( mode > 0 )
         cout<<"Simulation Mode : "<<mode<<endl;
      else 
         cout<<"Simulation Mode : "<<"default"<<endl;
   }
   if ( doFile && _mode )
      cerr<<"warning : mode selection has no effect"<<endl;
   assert (curCmd != CIRINIT);
   if (doLog)
...
CirSimCmd::usage(ostream& os) const
{
   os << "Usage: CIRSIMulate <-Random | -File <string patternFile>>\n"
      << "                   [-Output (string logFile)][-Mode (int mode)]" << endl;
}
...
//----------------------------------------------------------------------
//    CIRFraig [-All][-Repeat (int RepeatTimes)]
//----------------------------------------------------------------------
CmdExecStatus
CirFraigCmd::exec(const string& option)
{
   ...
   for (size_t i = 0, n = options.size(); i < n; ++i) {
      if (myStrNCmp("-Repeat", options[i], 2) == 0) {
         ...
         if (++i == n)
            return CmdExec::errorOption(CMD_OPT_MISSING, options[i-1]);
         if ( !( myStr2Int(options[i], repeatTimes) && repeatTimes >= 0 ) ) 
            return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[i]);
         hassetrepeat = true;
      }
      else if (myStrNCmp("-All", options[i], 2) == 0) 
         all = true;
      else return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[i]);
   }
   
   if ( repeatTimes == -1 && all == false ) repeatTimes = 1;
   if ( all == false && repeatTimes == 0 )
      cerr<<"warning : nothing will happen when fraiging"<<endl;
   if ( repeatTimes == -1 && all == true  ) repeatTimes = 0;
   cirMgr -> setRepeat( repeatTimes );
   cirMgr -> setAll( all );
   cirMgr -> fraig();
   curCmd = CIRFRAIG;
...
{
   os << "Usage: CIRFraig [-All][-Repeat (int RepeatTimes)]" << endl;
}
